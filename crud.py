...
import schemas
from db.enums import PackagingType
...
def get_choose_type_by_name(db: Session, name: str):
    return (
        db.query(models.DBCheeseType).filter(models.DBCheeseType.name == name).first()
    )


def create_choose_type(db: Session, cheese_type: schemas.CheeseTypeCreate):
