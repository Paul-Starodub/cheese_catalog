...
def create_cheese_type(
    ...
    db_choose_type = crud.get_choose_type_by_name(db=db, name=cheese_type.name)
    if db_choose_type:
        raise HTTPException(
            status_code=400, detail="Such name for Cheese Type already exists"
        )
    return crud.create_choose_type(db=db, cheese_type=cheese_type)
